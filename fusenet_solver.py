...
from models.uniform_net import FuseNet
...
class Solver(object):
    ...
    def reset_histories_and_losses(self):
        ...
        #self.states['train_depth_acc_hist'] = []
        #self.states['val_depth_acc_hist'] = []
    ...
    def save_checkpoint(self, state, lam, is_best):
        ...
        if self.use_class:
            lam_text = ('_depth_completion_' + '%.5f' % lam).replace('.', '_')
        else:
            lam_text = ''
        #lam_text = ''
        #lam_text = ('_depth_completion_' + '%.5f' % lam).replace('.', '_')
...
#need to modify carefully
    def validate_model(self, val_loader, vis_results=False, outTrain=False):
        ...
        for i, batch in enumerate(val_loader):
            ...
            val_d_inputs = Variable(batch[3].cuda(self.gpu_device))  #why [1] doesn't work
            val_labels = Variable(batch[2].cuda(self.gpu_device))

            print('[PROGRESS] Processing images: %i of %i    ' % (i+1, len(val_loader)), end='\r')

            if self.use_class:
                val_depth_labels = Variable(batch[3].cuda(self.gpu_device))
                ...
                # val_preds_class += 1
                # val_class_scores.append(np.mean(val_preds_class.data.cpu().numpy() == val_depth_labels.data.cpu().numpy()))
            else:
    ...
    def train_model(self, train_loader, val_loader, num_epochs=10, log_nth=0, lam=None):
        ...
        lam: torch.float32
            lambda value used as weighting coefficient for depth completion loss
        """
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            running_loss = []
            running_depth_loss = []   #running_class_loss
            running_seg_loss = []
            train_seg_scores = []
            train_depth_scores = []   #train_class_scores
            ...
            for i, data in enumerate(train_loader):
                ...
                d_inputs = Variable(data[3].cuda(self.gpu_device))
                train_seg_labels = Variable(data[2].cuda(self.gpu_device))
                #train_depth_labels = Variable(data[1].cuda(self.gpu_device))

                if self.use_class:
                    depth_labels = Variable(data[1].cuda(self.gpu_device))
                    # forward + backward + optimize with segmentation and depth completion loss
                    output_seg, output_depth_completion = self.model(rgb_inputs, d_inputs)
                    #transfer torch.cuda.FloatTensor to torch.cuda.LongTensor
                    output_depth_completion=output_depth_completion.long()
                    loss, seg_loss, depth_loss = criterion(output_seg, train_seg_labels, output_depth_completion, depth_labels, lambda_2=lam)
                else:
                ...
                if self.use_class:
                    ...
                    running_depth_loss.append(depth_loss)

                    #_, train_class_preds = torch.max(output_depth_completion, 1)  #?

                    #train_class_preds += 1
                    #train_depth_scores.append(np.mean((train_class_preds == depth_labels).data.cpu().numpy()))
                    del depth_labels  #, train_class_preds
                ...
                if (i+1) % log_nth == 0 or (i+1) == iter_per_epoch:
                    ...
                    if self.use_class:
                        ...
                        print("\r[Epoch: %d/%d Iter: %d/%d] Total_Loss: %.3f Seg_Loss: %.3f "
                              "depth_loss: %.3f Best_Acc(IoU): %.3f LR: %.2e Lam: %.5f Time: %.2f seconds         "
                              % (epoch + 1, end_epoch, i + 1, iter_per_epoch, loss_log_nth, seg_loss_log_nth,
                                 depth_loss_log_nth, self.states['best_val_seg_acc'], optim.param_groups[0]['lr'], lam,
            ...
            if self.use_class:
                ...
                self.states['train_depth_loss_hist'].append(np.mean(running_depth_loss))
                # print('Train Class Scores shape and itself: ', len(train_depth_scores), train_depth_scores)
            ...
            # if self.use_class:
            #     train_class_acc = np.mean(train_depth_scores)
            #     self.states['train_class_acc_hist'].append(train_class_acc)
						#
            #     print('[INFO] TRAIN Seg_Acc: %.3f Class_Acc: %.3f Loss: %.3f Seg_Loss: %.3f depth_loss: %.3f Epoch: %d/%d'
            #           % (train_seg_acc, train_class_acc, self.states['train_loss_hist'][-1],
            #              self.states['train_seg_loss_hist'][-1], self.states['train_depth_loss_hist'][-1],
            #              epoch + 1, end_epoch))
            # else:
